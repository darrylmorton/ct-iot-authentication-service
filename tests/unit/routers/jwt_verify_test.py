...
class TestJwtVerify:
    ...
    async def test_valid_token(self):
        response = await RoutesHelper.http_client(
            server, RoutesHelper.TEST_URL, "/api/jwt", self.token
        )
    ...
    async def test_missing_token(self):
        response = await RoutesHelper.http_client(
            server, RoutesHelper.TEST_URL, "/api/jwt", {}
        )
    ...
    async def test_expired_token(self):
        expired_token = jwt.encode(
            {"id": self.id, "exp": JwtHelper.create_token_expiry(-1)},
            JWT_SECRET,
            algorithm="HS256",
        )

        response = await RoutesHelper.http_client(
            server, RoutesHelper.TEST_URL, "/api/jwt", expired_token
        )
    ...
    async def test_invalid_token_id(self):
        ...
        response = await RoutesHelper.http_client(
            server, RoutesHelper.TEST_URL, "/api/jwt", invalid_token
        )
